...
**Goal**: Create a sandbox system where dcmaidbot can create using tools in tg for admins interactive webapp games with token-protected admin-only access, event emitter system, and p2p realtime game creation capabilities. (can be verified with /call at local/dev/prod)
...
### Phase 1: Foundation & Security (1 day) - ✅ COMPLETE
...
### Phase 2: Event System Implementation (1 day) - ✅ COMPLETE
...
### Phase 3: Webapp Interface (1 day) - ✅ COMPLETE
...
### Phase 4: Domik Tool & Integration (1 day) - ✅ COMPLETE
...
### Phase 5: Webapp Server Integration (1 day) - ✅ COMPLETE
...
```
dcmaidbot/
├── static/tgapp/                  # Telegram webapp directory
│   ├── index.html                 # Main webapp interface
│   ├── style.css                  # Child-like naive styling
│   ├── app.js                     # Event emitter and Telegram SDK
│   └── assets/                    # Images and resources
...
│       └── backgrounds/           # Playful backgrounds
├── services/
│   ├── event_service.py           # Existing event service
│   ├── token_service.py           # Token authentication (new)
│   └── domik_service.py           # Sandbox file operations (new)
├── models/
│   ├── event.py                   # Existing event model
│   └── admin_token.py             # Token model (new)
├── api/
│   ├── events.py                  # Event collection endpoint
│   └── admin.py                   # Admin webapp endpoints
├── tools/
│   └── domik_tool.py              # dcmaidbot integration tool
└── tests/
    ├── e2e/test_webapp_events.py  # Webapp event flow tests
    └── unit/test_token_auth.py    # Token security tests
```
