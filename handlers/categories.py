...
async def process_pool_name(message: Message, state: FSMContext):
    ...
    if existing_pool:
        await message.answer(
            f"Пул с названием '<b>{pool_name}</b>' уже существует. "
    ...
    if new_pool:
        await message.answer(
            f"✅ Пул '<b>{pool_name}</b>' успешно создан! "
            "Вы являетесь его первым участником и создателем.\n\n"
            ...
            "Другие пользователи могут присоединиться только по вашему приглашению.",
            parse_mode="HTML",
        )
    else:
        await message.answer(
            "❌ Не удалось создать пул. Пожалуйста, попробуйте снова.",
            parse_mode="HTML",
        )
...
async def process_join_with_code(
    message: Message, state: FSMContext, pool_name: str, code: str
):
    # Check if pool exists
    pool = pool_service.get_pool(pool_name)
    if not pool:
        await message.answer(
            f"❌ Пул с названием '<b>{pool_name}</b>' не найден. "
    ...
    for participant in pool.participants:
        if participant.user_id == message.from_user.id:
            await message.answer(
                f"Вы уже являетесь участником пула '<b>{pool_name}</b>'.",
                parse_mode="HTML",
            )
            await state.clear()
            return
    ...
    if success:
        await message.answer(
            f"✅ Вы успешно присоединились к пулу '<b>{pool_name}</b>'!",
            parse_mode="HTML",
        )
        ...
        try:
            ...
            if pool_name in storage.pools and hasattr(
                storage.pools[pool_name], "invites"
            ):
        ...
        await message.answer(
            "❌ Не удалось присоединиться к пулу. Пожалуйста, попробуйте снова.",
            parse_mode="HTML",
        )
...
async def process_join_pool(message: Message, state: FSMContext):
    ...
    if not pool_service.is_user_authorized_for_pool(pool_name, message.from_user.id):
        await message.answer(
            f"❌ Для присоединения к пулу '<b>{pool_name}</b>' требуется код "
            "приглашения от создателя пула.\n"
            "Используйте команду /join_pool с кодом приглашения.",
            parse_mode="HTML",
        )
        await state.clear()
        return
...
async def cmd_exit_pool(message: Message, state: FSMContext):
    ...
    if not user_pools:
        await message.answer(
            "Вы не являетесь участником ни одного пула.", parse_mode="HTML"
        )
        return

    pool_list = "\n".join(
        [f"{i+1}. <b>{pool.name}</b>" for i, pool in enumerate(user_pools)]
    )
...
@router.message(
    F.text.regexp(r"^\d+$"), PoolParticipant.waiting_for_exit_pool_selection
)
async def process_exit_pool(message: Message, state: FSMContext):
    logging.info(
        f"User {message.from_user.id} entered pool index for exit: {message.text}"
    )
    ...
    try:
        ...
        if 0 <= index < len(user_pools):
            ...
            success = pool_service.remove_participant(
                selected_pool.name, message.from_user.id
            )

            if success:
                await message.answer(
                    f"✅ Вы успешно вышли из пула '<b>{selected_pool.name}</b>'.",
                    parse_mode="HTML",
                )
            else:
                await message.answer(
                    "❌ Не удалось выйти из пула. Пожалуйста, попробуйте снова.",
                    parse_mode="HTML",
                )
        else:
            await message.answer(
        ...
        await message.answer(
            "❌ Пожалуйста, введите номер пула из списка.", parse_mode="HTML"
        )
...
async def cmd_my_pools(message: Message):
    ...
    response += (
        "Используйте /pool_info [название пула] " "для просмотра подробной информации."
    )
...
async def cmd_invite(message: Message, state: FSMContext):
    ...
    if not user_pools:
        await message.answer(
            "У вас нет созданных пулов. " "Создайте пул с помощью команды /create_pool",
            parse_mode="HTML",
        )
        return
    ...
    await message.answer(
        "Выберите пул, в который хотите пригласить участника:",
        reply_markup=markup,
        parse_mode="HTML",
    )
...
async def process_invite_pool_selection(message: Message, state: FSMContext):
    ...
    if not pool:
        await message.answer(
            "Пул не найден или вы не являетесь его создателем. "
    ...
    try:
        ...
        await message.answer(
            f"Ошибка при генерации кода приглашения: {str(e)}",
            reply_markup=ReplyKeyboardRemove(),
            parse_mode="HTML",
        )
