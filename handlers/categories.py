...
async def process_join_with_code(
    ...
    if success:
        ...
        try:
            ...
            if (
                pool_name in storage.pools
                and hasattr(storage.pools[pool_name], "invites")
            ):
...
async def cmd_my_pools(message: Message):
    ...
    response += (
        "Используйте /pool_info [название пула] "
...
async def cmd_invite(message: Message, state: FSMContext):
    ...
    if not user_pools:
        await message.answer(
            "У вас нет созданных пулов. "
            "Создайте пул с помощью команды /create_pool",
            parse_mode="HTML",
        )
        return
