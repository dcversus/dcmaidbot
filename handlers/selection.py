...
async def process_direct_selection(message: Message, state: FSMContext, indices: list):
    ...
    if activity.media and len(activity.media) > 0:
        logging.info(
            f"Activity has {len(activity.media)} media items. Attempting to send."
        )
        for media_id in activity.media:
            try:
                ...
                logging.warning(
                    f"Failed to send media {media_id} as photo: {e}. "
                    "Maybe it's a document or invalid? Skipping."
                )
                ...
                #     logging.error(
                #         f"Failed to send media {media_id} as document either: {doc_e}"
                #     )
