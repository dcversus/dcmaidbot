...
async def handle_web_search(message: Message, state: FSMContext):
    ...
    try:
        ...
        if results and results.get('success'):
            ...
            error_msg = results.get('error', 'Unknown error occurred') if results else 'Search service unavailable'
            await message.reply(
                f"❌ **Search Failed**\n\n"
                f"Error: {error_msg}\n\n"
...
async def handle_curl_request(message: Message, state: FSMContext):
    ...
    try:
        ...
        if result and result.get('success'):
            ...
            error_msg = result.get('error', 'Unknown error occurred') if result else 'Request failed'
            await message.reply(
                f"❌ **Request Failed**\n\n"
                f"Error: {error_msg}\n\n"
