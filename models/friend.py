...
class Friendship(Base):
    ...
    status = Column(String(20), nullable=False, default='pending', comment='pending/active/blocked')
    created_at = Column(DateTime, nullable=False, default=datetime.utcnow)
    updated_at = Column(DateTime, nullable=False, default=datetime.utcnow, onupdate=datetime.utcnow)
    message = Column(Text, nullable=True, comment='Optional message with friend request')
    blocked_reason = Column(Text, nullable=True, comment='Reason for blocking if status=blocked')
...
class FriendRequest(Base):
    ...
    status = Column(String(20), nullable=False, default='pending', comment='pending/accepted/declined/ignored')
    message = Column(Text, nullable=True, comment='Optional message with request')
    created_at = Column(DateTime, nullable=False, default=datetime.utcnow)
    responded_at = Column(DateTime, nullable=True, comment='When request was responded to')
