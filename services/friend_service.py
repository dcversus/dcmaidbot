...
class FriendService:
    ...
    async def accept_friend_request(self, user_id: int, request_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Friendship created between {request['from_user']} and {request['to_user']}")
    ...
    async def _check_friendship(self, user_id: int, friend_id: int) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to check friendship between {user_id} and {friend_id}: {e}")
            return False
