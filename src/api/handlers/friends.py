...
async def handle_pending_requests(message: Message, state: FSMContext):
    ...
    try:
        ...
        response += (
            "💡 **Tip:** To accept a request, use: `/accept_request <request_id>`"
        )
...
async def handle_accept_request(message: Message, state: FSMContext):
    ...
    try:
        ...
        if result["success"]:
            ...
            response += (
                f"🆔 **Friendship ID:** `{result.get('friendship_id', 'N/A')}`\n\n"
            )
            response += (
                "You are now friends! View your full friends list with `/friends`"
            )
        else:
