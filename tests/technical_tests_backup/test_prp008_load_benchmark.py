...
**Overall Grade**: {"A" if all(self.results.get(comp, {}).get("messages_per_second", 0) > threshold for comp, threshold in [("chat_buffer", 500), ("memory_implicator", 100), ("response_flow", 1000)]) else "B" if sum(1 for comp, threshold in [("chat_buffer", 500), ("memory_implicator", 100), ("response_flow", 1000)] if self.results.get(comp, {}).get("messages_per_second", 0) > threshold) >= 2 else "C"}
