...
async def test_handle_create_pool_command(mock_create_pool, mock_message, mock_state):
    ...
    mock_state.set_state.assert_called_once_with(
        categories.PoolCreation.waiting_for_name
    )
...
async def test_handle_exit_pool_command(
    ...
    mock_state.set_state.assert_called_once_with(
        categories.PoolParticipant.waiting_for_exit_pool_selection
    )
...
async def test_handle_add_activity_command(mock_get_pools, mock_message, mock_state):
    ...
    # Check that state was set and user_pools was stored
    mock_state.set_state.assert_called_once_with(
        activities.ActivityManagement.selecting_pool
    )
    mock_state.update_data.assert_called_once()
    ...
    mock_state.set_state.assert_called_once_with(
        activities.ActivityManagement.entering_activity_content
    )
    ...
    with patch("services.activity_service.add_activity", return_value=True):
        # Process activity content using the renamed handler
        await activities.process_activity_input(mock_message, mock_state)
...
async def test_handle_selection_command(mock_get_pools, mock_message, mock_state):
    ...
    mock_state.set_state.assert_called_once_with(
        selection.ActivitySelection.selecting_pools
    )
    mock_state.update_data.assert_called_once()
