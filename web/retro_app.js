...
class RetroGameInterface {
    constructor() {
        this.floorIndicators = new Map();
        this.locationPanels = new Map();
        this.overlayPanels = new Map();
        this.widgetStates = new Map();
        this.animationFrame = 0;
        this.isScrolling = false;
        this.currentFloorValue = 2; // Start on floor 2
    ...
    async loadLocation(locationId) {
        try {
            ...
            for (const floor of this.appState.worldConfig?.floors || []) {
                const location = floor.locations.find((loc) => loc.id === locationId);
                if (location) {
    ...
    handleFloorNavigation(direction) {
        const floors = ['B1', '1F', '2F', 'F1', 'F2', 'F3', 'F4', 'F5'];
        const currentIndex = floors.findIndex(floor => this.floorIndicators.get(floor)?.classList.contains('active'));
        if (currentIndex !== -1) {
    ...
    handleGenericWidgetClick(widgetId, currentState) {
    ...
    showOverlayPanel(widgetId, type, content) {
    ...
    async loadMarkdownContent(panel, url) {
        try {
